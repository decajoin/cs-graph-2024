...
// 变换函数：实现关于（10， 15）点顺时针旋转 90 度
void transformation(int &x, int &y, double k)
{
    // 先将点平移，使 (10, 15) 变为原点
    int translated_x = x - 10;
    int translated_y = y - 15;
    ...
    // 将原始坐标 (x, y) 表示为一个列向量
    double original_coords[2] = {static_cast<double>(translated_x), static_cast<double>(translated_y)};
    ...
    // 将计算结果转换为整数并更新坐标
    int rotated_x = round(new_x);
    int rotated_y = round(new_y);
...
void draw_transformed_graph(const int *g, int length)
{
    for (int i = 0; i < length; i += 4)
    {
        ...
        // 图像变换
        transformation(x1, y1, 90);
        transformation(x2, y2, 90);
